...
import { getErpProcessById, updateErpProcessById } from 'apiSdk/erp-processes';
...
function ErpProcessEditPage() {
  ...
  const handleSubmit = async (values: ErpProcessInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateErpProcessById(id, values);
      mutate(updated);
      resetForm();
