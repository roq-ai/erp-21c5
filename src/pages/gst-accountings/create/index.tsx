...
import { createGstAccounting } from 'apiSdk/gst-accountings';
...
function GstAccountingCreatePage() {
  ...
  const handleSubmit = async (values: GstAccountingInterface, { resetForm }: FormikHelpers<any>) => {
    setError(null);
    try {
      await createGstAccounting(values);
      resetForm();
